...
def handle_message_event(ack, event, client):
    ...
    if user_utterance.startswith(pattern):
        ...
        rewrite_output = rewrite_output.split("\n")
        rewrite_thought = rewrite_output[0].lstrip("判断思路：").strip()
        rewrite_query = rewrite_output[1].lstrip("重写查询：").strip()
            ...
            if "不需要提出澄清性问题" not in clarify_question:
                ...
                memory.write_into_memory(
                    ...
                    utterance_info={"speaker": "CoSearchAgent", "utterance": clarify_question, "convs": convs,
                                    "query": query, "rewrite_query": rewrite_query, "rewrite_thought": rewrite_thought,
